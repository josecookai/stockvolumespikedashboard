...
interface MarketVolumeResponse {
  totalUsd: number;
...
export default function HomePage() {
  ...
  const [volumeData, setVolumeData] = useState<MarketVolumeResponse | null>(null);
  ...
  const stockVolume = volumeData?.totalUsd ?? data?.total_stock_volume ?? 900_000_000_000;
  ...
  return (
    <div className="space-y-5">
      ...
      <MarketSummaryBar
        totalCryptoVolume={data?.total_crypto_volume ?? 100_000_000_000}
        totalStockVolume={stockVolume}
        updatedAt={volumeData?.fetchedAt ?? new Date().toISOString()}
        nasdaqSource={volumeData?.nasdaqSource}
        cboeSource={volumeData?.cboeSource}
      />
