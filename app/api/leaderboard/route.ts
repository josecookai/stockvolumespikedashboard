...
import { getStocksLeaderboard, getStocksMockLeaderboard } from '@/lib/data/stocks-aggregator';
import { getTotalUSMarketVolume } from '@/lib/data/market-volume';
...
export async function GET(request: Request) {
  ...
  // Fetch real stock data; fall back to mock on failure
  let stockRows: Awaited<ReturnType<typeof getStocksLeaderboard>>;
  let usingMock = false;
  try {
    stockRows = await getStocksLeaderboard();
  } catch {
    const totalUsd = await getTotalUSMarketVolume().catch(() => 900_000_000_000);
    stockRows = getStocksMockLeaderboard(totalUsd);
    usingMock = true;
  }

  const cryptoAssets = CRYPTO_ASSETS;
  const allAssets = [...cryptoAssets, ...stockRows];
  ...
  const cryptoTotal = cryptoAssets.reduce((s, a) => s + a.volume_usd, 0);
  const stockTotal  = stockRows.reduce((s, a) => s + a.volume_usd, 0);

  return NextResponse.json({
    items,
    total_crypto_volume: Math.round(cryptoTotal * rangeMultiplier),
    total_stock_volume:  Math.round(stockTotal  * rangeMultiplier),
    updated_at: new Date().toISOString(),
    range,
    count: items.length,
    stock_data_source: usingMock ? 'mock' : 'yahoo_finance',
  });
}
